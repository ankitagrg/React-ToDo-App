...
useEffect(()=>{
  ...
  return(
    ...
    flex flex-col p-7 min-h-[600px] rounded-md'>
     ...
     <div className='flex items-center mt-7 gap-2'>
        ...
        <button onClick={add} className='border-none rounded-md bg-blue-400 w-32 h-14 text-white text-lg font-medium 
      ...
      {todoList.map((item, index)=>{
        return<TodoItems 
        key={item.id} text={item.text} id={item.id} isComplete={item.isComplete} deleteTodo={deleteTodo} toggle={toggle} />
      })}
